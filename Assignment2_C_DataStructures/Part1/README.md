...
# TODO: Put your answer here
	...
	A ring buffer is useful when it is needed to have inputs and processes done at the same time. The ring buffer helps by allowing for a more smaller memory usage for systems, saving harddisk space as it overwrites old memory. It should be used whenever processes with changing user inputs are needed and if the memory space must be conservative.
	In example, if a program used a vast amount of memory, and it was required to save some memory, you would use a ring buffer, which uses the same small amount of memory space for writing, reading, and rewriting data over the available memory slots. This is useful for when the program is processing and writing data at the same time.
