...
# TODO: Put your answer here
	...
	A stack is useful for retreiving data in a LIFO sense. It helps if you need to obtain data in the last way you put it on, as compared to an array which accesses it randomly. It should be used when you are using a tree structure or data strucutre that is built from the ground up, allowing you to take the last stored information for usage. Lastly, there is the time efficiency of stacks. They process data at O(1), instead of O(n), making them much faster for larger arrays.
	In example, when creating a tree data structure, you would begin by creating the "trunk". Then as it continues up, it would create each branch with data. When data is needed, it would "pop" the last branch created and move down to the next lower branch.
