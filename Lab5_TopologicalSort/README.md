...
1. The space complexity would require n^2 of space to keep track of connections. This could take up much space as one must also account for all spaces using 0.
2. Yes, adjacency matrixes may be unsymmetrical if a directed graph has points not pointing back to upwards nodes.
...
1. While you can "techically" run a tsort on an undirected graph, it won't work as expected as there is no privileged nodes to help direct the tsort. As for running into cycles in a directed graph, one could use flags in an algorithm to help detect previously visited nodes while on a path. Otherwise, an algorithm that kept track of the "parent" or starting node for the path would help to detect if a cycle exists as the path relooped to the parent node.
