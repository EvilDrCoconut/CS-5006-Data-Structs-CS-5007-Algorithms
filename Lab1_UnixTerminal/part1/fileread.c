...
int main(){
	...
	// We should check if we successfully opened to the next file.
	...
	// We will read each line, until we reach the eof of the file (EOF)
	char buffer[255]; // At most, each line in file can be 255 characters
	// So we will now can each line from a file(fscanf),
	// and stroe it as a character string(%s) in buffer each iteration
