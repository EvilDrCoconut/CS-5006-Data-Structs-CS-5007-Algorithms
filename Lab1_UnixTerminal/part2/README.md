...
The registers used to hold the longs, and conduct the swaps, are twice the size on the ints. Going from 4 or 8 bytes, to 8 to 16 bytes.
...
This program calls upon another function, the swap function. it then uses the same registers inside the swap function to switch the values between the two integers.
...
The offsets for the addresses are between 6 and -16, a total offset of 22 bytes.
...
A dynamic array calls upon the malloc function, and as it sets values in allotted array places, it uses move and add functions instead of just move functions. So more of pointing to its address.
...
The type of jmp statement used is a jne, which is automatically directed to the correct case choice.
...
The condition statement is near the beginning of the main. It checks if the register containing i holds a value greater than 6, if not then it conitues into .L3, otherwise it skips to .L2 and ends the program.
...
It creates the switch cases' output above the main, and then in the main runs the comparison for the correct case and gives case output before moving to .L6 and ending program.
...
The popq is called in the add function, at the end where the newly added value is returned. It seems to queue the numbers, and pops its overall added value.
