...
int findMinimum(int* array, int start, int stop){
	int ptr = start;
	int number = array[ptr];
	int i = stop - start;
	int arrayCheck = start + 1;
	int check = array[arrayCheck];
	for(;i > 0; i --){
		if(check < number){
			ptr = arrayCheck;
			number = array[ptr];	
    ...
    return ptr; // TODO: Modify this to return the
}
...
void sortIntegers(int* array, unsigned int size){
	...
	int start = 0;
	for(;i > 0; i --){
		int minimum = findMinimum(array, start, (size - 1));
		int* a = &array[start];
		int* b = &array[minimum];
		swap(a, b);
		start ++;	
