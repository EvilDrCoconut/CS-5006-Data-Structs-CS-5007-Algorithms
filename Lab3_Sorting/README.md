...
1. The Big-Og worst case time complexity would be O(n^2) for if every element has to be sorted.
2. The best-case complexity would be O(n), if the array happened to already be sorted and it simply has to check to make sure it is sorted.
3. No it doesn't require any additional storage beyond the array to hold information.
