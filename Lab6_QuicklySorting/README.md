...
The strcmp function is implemented by testing the first character of each string being compared. As shown by the above chart, whichever character has a lower decimal value, or hexadecimal value is put first. however, if two strings have the same first character, then their second characters are compared to each other.
