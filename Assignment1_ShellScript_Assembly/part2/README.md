...
1. The bug was a segmentation fault as register eax does not exist..
2. The bug occured in line 15.
3. To fix the bug, memory is not allocated for a value on a stack, so when attempting to allocate value for a new block, it attempts to assign a value without the memory space allocated.
...
Pro:
1. This helps to find code flaws before the program is utilized.

Con:
1. The tools only scan the code and miss weakpoints which may cause issues.
...
Pro:
1. A pro with dynamic analysis would be that it finds errors and leaks with the program while running. 

Con:
1. It can be difficult to find the faults and vulnerabilities in a program, and longer to fix.
