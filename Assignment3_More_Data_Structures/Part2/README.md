...
1. The worst Big -Oh case would be O(n^2) if it had to check through every elemnet of every cycle. Pretty much if the lowest element was last, then the second lowest elemenet was last, and so on.
2. The best case complexity would be O(n) as it would shuffle through to the second loop, but since the next number is already bigger, no searching is needed.
3. Insertion sort does not require any additional memory space, as all the searching is done inside the function. The search itself does not need to allocate any variables as they are temporary variables to assist with the searching. The swap helped function only uses a temperary variable for that function, not allocating on the heap. 
4. A stable sorting algorithm is one where when variables inside an array have the same key value, produces the same output always as that to the sorted array. An unstable array may change variables with the same key value in different orders. Insertion sort is one of the stable algorithms that can be used.
