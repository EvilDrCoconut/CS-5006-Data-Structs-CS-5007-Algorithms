...
Starvation may occur if two of the rookie artists where randomly given the same starting position. The first artist through the lock would paint it, forcing the second artist to "end" its painting session. To fix this, you would want to set an if statement and check if any other rookie artist has the same positon, if so, then retry the process. A while loop would also work.
